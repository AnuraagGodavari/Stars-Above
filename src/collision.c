...
Bool box_detectCollision(Collider_Box* box0, Collider_Box* box1)
{
	...
	if (
		(box0->worldpos.x + box0->size.x > box1->worldpos.x) &&		//R0 bottom right x > R1 top left x
		(box0->worldpos.x < box1->worldpos.x + box1->size.x) &&		//R0 top left x > R1 bottom right x
		(box0->worldpos.y + box0->size.y > box1->worldpos.y) &&		//R0 bottom right y > R1 top left y
		(box0->worldpos.y < box1->worldpos.y + box1->size.y)		//R0 top left y > R1 bottom right y
		)
	{
		return 1;
	}
		...
		(box0->worldpos.x + box0->size.x > box1->worldpos.x) &&		//R1 bottom right x > R0 top left x
		(box0->worldpos.x < box1->worldpos.x + box1->size.x) &&		//R1 top left x > R0 bottom right x
		(box0->worldpos.y + box0->size.y > box1->worldpos.y) &&		//R1 bottom right y > R0 top left y
		(box0->worldpos.y < box1->worldpos.y + box1->size.y)		//R1 top left y > R0 bottom right y
		)
	{
		return 1;
	}
